...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    if (freq.size() < 2) error("the string must have two different character");
    PriorityQueue<EncodingTreeNode*> pq;
    for (char character : freq) {
        EncodingTreeNode* node = new EncodingTreeNode;
        node->ch = character; node->zero = nullptr; node->one = nullptr;
        pq.enqueue(node, freq[character]);
    }
    while (pq.size() != 1) {
        int freq1 = pq.peekPriority();
        EncodingTreeNode* left = pq.dequeue();
        int freq2 = pq.peekPriority();
        EncodingTreeNode* right = pq.dequeue();
        EncodingTreeNode* newNode = new EncodingTreeNode;
        newNode->zero = left; newNode->one = right;
        pq.enqueue(newNode, freq1 + freq2);
    }
    return pq.dequeue();
}
...
bool isLeaf(EncodingTreeNode* node) {
    if (node->zero == nullptr && node->one == nullptr) return true;
    return false;
}
 ...
 */
void huffTable(EncodingTreeNode* tree, Map<char, string>& table
                                , string path) {
    if (isLeaf(tree)) {
        table[tree->ch] = path;
        return;
    }
    huffTable(tree->zero, table, path + "0");
    huffTable(tree->one, table, path + "1");
}

Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    Map<char, string> table;
    Queue<Bit> result;
    huffTable(tree, table, "");
    for (char character : str) {
        for (char bit : table[character]) {
            if (bit == '0') result.enqueue(0);
            else if (bit == '1') result.enqueue(1);
        }
    }
    return result;
}
