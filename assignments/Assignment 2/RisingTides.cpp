...
bool floodable(const Grid<double>& terrain, GridLocation& loc, double& height) {
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for (GridLocation source : sources) {
        if (floodable(terrain, source, height)) {
            q.enqueue(source);
            result.set(source, true);
        }
    }
    GridLocation loc, up, down, left, right;
    while (!q.isEmpty()) {
        loc = q.dequeue();
        up = {loc.row - 1, loc.col};
        down = {loc.row + 1, loc.col};
        left = {loc.row, loc.col - 1};
        right = {loc.row, loc.col + 1};
        // result.set(loc, true); 这里会出错，必须要在进队时出队前标记 loc 为 true, 否则会重复 enqueue 已经访问过的点，陷入死循环
        if (floodable(terrain, up, height) && !result.get(up)) {
            q.enqueue(up);
            result.set(up, true);
        }
        if (floodable(terrain, down, height) && !result.get(down)) {
            q.enqueue(down);
            result.set(down, true);
        }
        if (floodable(terrain, left, height)&& !result.get(left)) {
            q.enqueue(left);
            result.set(left, true);
        }
        if (floodable(terrain, right, height) && !result.get(right)) {
            q.enqueue(right);
            result.set(right, true);
        }
    }
    return result;
}
