...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0) {
        error("the kGramLength can't less than zero");
    }
    string text = str;
    Map<string, double> result;
    while (kGramLength <= text.length()) {
        result[text.substr(0, kGramLength)] += 1;
        text = text.substr(1);
    }
    return result;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string text : input) {
        s += input[text] * input[text];
    }
    s = sqrt(s);
    if (s == 0) {
        error("no inputs or inputs are all zero");
    }
    for (string text : input) {
        result[text] = input[text] / s;
    }
    return result;
}
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        error("numToKeep can't less than zero");
    }
    Map<string, double> result;
    if (numToKeep == 0) {
        return result;
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (Corpus language : corpora) {
        pq.enqueue(language.name, 1 - cosineSimilarityOf(textProfile, language.profile));
    }
    return pq.dequeue();
}
